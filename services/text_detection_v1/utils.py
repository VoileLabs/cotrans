...
class Quadrilateral(object) :
	...
	def to_tref_and_normalize(self, w: int, h: int) -> TextRegionExchangeFormat :
		...
		lines = [l.to_tref_and_normalize(w, h) for l in self.lines]
		return TextRegionExchangeFormat(fmt = 'quad', coords = coords_base64, fg = fg, bg = bg, text = text, prob = prob, lines = lines)
	...
	def from_tref(tref: TextRegionExchangeFormat, w: int, h: int) :
		...
		ret.lines = [Quadrilateral.from_tref(l, w, h) for l in tref.lines]
		return ret
