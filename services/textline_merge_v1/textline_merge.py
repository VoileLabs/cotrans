...
def split_text_region(bboxes: List[Quadrilateral], region_indices: Set[int], gamma = 0.5, sigma = 2, std_threshold = 6.0) -> List[Set[int]] :
	...
	if (edge_weights[0] <= mean + std * sigma or edge_weights[0] <= fontsize * (1 + gamma)) and std < std_threshold :
		...
		for node_set in nx.algorithms.components.connected_components(G) :
			ans.extend(split_text_region(bboxes, node_set, gamma = gamma, sigma = sigma, std_threshold = std_threshold))
		return ans
	pass
...
def merge_bboxes_text_region(bboxes: List[Quadrilateral], width, height, gamma = 0.5, sigma = 2, std_threshold = 6.0) :
	G = nx.Graph()
	...
	for node_set in nx.algorithms.components.connected_components(G) :
		...
		region_indices.extend(split_text_region(bboxes, node_set, gamma, sigma, std_threshold))
...
from models import V1TextlineMergeRequest

def run_merge(cfg: V1TextlineMergeRequest, textlines: List[Quadrilateral], width: int, height: int) -> Tuple[List[Quadrilateral], str] :
	text_regions: List[Quadrilateral] = []
	new_textlines = []
	for (poly_regions, textline_indices, majority_dir, fg_r, fg_g, fg_b, bg_r, bg_g, bg_b) in merge_bboxes_text_region(textlines, width, height, cfg.gamma, cfg.sigma, cfg.std_threshold) :
		text = ''
		logprob_lengths = []
