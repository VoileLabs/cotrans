...
from models import V1InapintingLamaRequest
import numpy as np
...
def load_model(app: FastAPI) :
	model = LamaFourier(build_discriminator=False, use_mpe=True)
	sd = torch.load('inpainting_lama_mpe.ckpt', map_location = 'cpu')
	model.generator.load_state_dict(sd['gen_state_dict'])
	model.mpe.load_state_dict(sd['str_state_dict'])
	model.eval()
	app.package = {
...
def run_inpainting(app: FastAPI, cfg: V1InapintingLamaRequest, img: np.ndarray, mask: np.ndarray) -> Tuple[np.ndarray, str] :
	model: LamaFourier = app.package['model']
	...
	if max(img.shape[0: 2]) > cfg.inpainting_size :
		img = resize_keep_aspect(img, cfg.inpainting_size)
		mask = resize_keep_aspect(mask, cfg.inpainting_size)
	pad_size = 8
	h, w, c = img.shape
	if h % pad_size != 0 :
